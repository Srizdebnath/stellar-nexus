...
// 🛑 CONFIGURATION: PASTE YOUR NEW V4 CONTRACT ID HERE
...
function AppletModal({ applet, onClose, walletAddress }: { applet: any, onClose: () => void, walletAddress: string | null }) {
    ...
    // --- 2. REAL PAYMENT LOGIC (DEBUGGED VERSION) ---
...
export default function Home() {
  ...
  const runArtApplet = async () => {
    if (!artInput) { alert("Please enter text"); return; }
    setArtLoading(true);
    try {
      const client = new Client({
        networkPassphrase: Networks.TESTNET,
  ...
  const scrollToDemo = (id: number) => {
    ...
    if (id === 3) document.getElementById('demo-art')?.scrollIntoView({ behavior: 'smooth' });
  };
    ...
    <main className="min-h-screen bg-[#050505] text-white font-sans selection:bg-blue-500/30">
      ...
      <div className="max-w-6xl mx-auto px-6 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {/* Demo 3: ASCII Art Generator (NEW) */}
          <div id="demo-art" className="group relative bg-[#0F0F11] border border-white/5 rounded-2xl p-6 hover:border-green-500/50 transition-all duration-300">
            <div className="absolute top-4 right-4 bg-green-500/10 text-green-400 text-xs px-2 py-1 rounded border border-green-500/20">Connected</div>
            <h3 className="text-xl font-bold mb-2">ASCII Art Gen</h3>
            <p className="text-gray-400 text-sm mb-6">Generates retro ASCII frames on-chain.</p>
            <div className="space-y-4">
               <input type="text" placeholder="Enter text..." className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-green-500" value={artInput} onChange={(e) => setArtInput(e.target.value)} />
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono text-gray-500">ID: {CONTRACT_ID.slice(0, 6)}...</span>
                <span className="text-green-400 font-bold text-sm">0.01 XLM</span>
              </div>
              <button onClick={runArtApplet} disabled={artLoading} className="w-full bg-green-900/50 hover:bg-green-800 text-green-100 border border-green-500/30 py-2.5 rounded-lg font-medium transition flex justify-center items-center">
                {artLoading ? "Generating..." : "Generate Art 🎨"}
              </button>
            </div>
            {artResult.length > 0 && (
                <div className="mt-4 p-4 bg-black border border-green-500/30 rounded-lg overflow-hidden font-mono text-xs text-green-400 leading-tight flex flex-col items-center">
                    {artResult.map((line, i) => <div key={i}>{line}</div>)}
                </div>
            )}
          </div>
